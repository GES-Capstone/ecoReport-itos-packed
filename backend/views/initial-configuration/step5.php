                ...
                <div class="card-body d-flex flex-column justify-content-between">
                    <div>
                        ...
                        <?= $form->field($modelFleet, 'area_id')->dropDownList(
                            ArrayHelper::map(
                                Area::find()->where(['mining_group_id' => $modelFleet->mining_group_id])->with(['miningProcess.company'])->all(),
                                'id',
                                function ($area) {
...
<?php
$areaData = [];
foreach (Area::find()->where(['mining_group_id' => $modelFleet->mining_group_id])->with(['miningProcess.company'])->all() as $area) {
    $areaData[$area->id] = [
        'areaName' => $area->name,
...
// Actualizar jerarquía al seleccionar área
